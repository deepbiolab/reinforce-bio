"""Visualization utilities for bioreactor optimization results.
...
def plot_predicted_profile(X_true, X_pred, X_columns, select_runs=[0], height=1000, predict_mode=False):
    """Plot predicted profiles for process variables.
    
    Args:
        X_true: True values (can be None in predict mode)
        X_pred: Predicted values
        X_columns: List of column names
        select_runs: List of run indices to plot
        height: Height of the plot
        predict_mode: Whether to plot in prediction mode (no true values)
    """
    max_cols_per_row = 5
    num_columns = len(X_columns)
    num_rows = (num_columns + max_cols_per_row - 1) // max_cols_per_row
    
    title = "Predicted process variables" if predict_mode else "Process variable evolution for selected runs"
    ...
    for idx, j in enumerate(select_runs):
        ...
        for i, c in enumerate(X_columns):
            ...
            # Only add true values trace if not in predict mode and X_true is provided
            if not predict_mode and X_true is not None:
                fig.add_trace(
                    go.Scatter(
                        x=list(range(15)),
                        y=X_true[:, i, j],
            ...
            fig.add_trace(
                go.Scatter(
                    x=list(range(15)),
                    y=X_pred[:, i, j],
                    name=f"Run {j} {'Prediction' if predict_mode else 'Predicted'}",
                    line=dict(dash="dash"),
                    marker=dict(color=color),
                    showlegend=show_legend,
                    legendgroup=f"group_{j}",
...
def plot_validation_comparison(optimal_result, X_train, X_test, X_columns, time_points):
    ...
    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    fig.suptitle(
        "Optimal Results vs Training/Test Data Distribution", fontsize=16, y=1.05
    )
    ...
    for i in range(4):
        ...
        for j in range(len(X_train)):
            axes[i].plot(
                time_points, X_train[j, :, i], color=train_color, alpha=0.2, linewidth=1
            )

        # Plot test data with transparency
        for j in range(len(X_test)):
            axes[i].plot(
                time_points, X_test[j, :, i], color=test_color, alpha=0.2, linewidth=1
            )
        ...
        axes[i].plot(
            time_points, train_mean, "--", color="gray", linewidth=2, label="Train Mean"
        )
        axes[i].plot(
            time_points, test_mean, "--", color="blue", linewidth=2, label="Test Mean"
        )

        # Plot optimal prediction
        axes[i].plot(
            time_points,
            optimal_result["predictions"][:, i],
            color=optimal_color,
            linewidth=2.5,
            label="Optimal",
        )
...
def create_optimization_animation(
    ...
    for param, (min_val, max_val) in param_ranges.items():
        param_normalizers[param] = lambda x, min_val=min_val, max_val=max_val: (
            x - min_val
        ) / (max_val - min_val)
    ...
    def update(frame):
        ...
        line1.set_data(anim_data[frame]["time_points"], anim_data[frame]["predictions"])
        scatter1.set_offsets([[time_points[-1], anim_data[frame]["final_titer"]]])

        # Update optimization progress plot
        line2.set_data(
            range(len(anim_data[frame]["best_titers"])), anim_data[frame]["best_titers"]
        )
    ...
    anim = FuncAnimation(fig, update, frames=len(anim_data), interval=100, blit=True)
