...
class EmbeddingRBF(StationaryKernelMixin, NormalizedKernelMixin, Kernel):
    @classmethod
    def make4EntityEmbed(
        cls, n_entities, embedding_dimension, flat_matrix_bounds=(0.1, 5.0)
    ):
        """
        ...
        flat_matrix = (
            np.random.rand(embedding_dimension * n_entities) * (mmax - mmin) + mmin
        )
    ...
    def vec_to_W(self, vec):
        return vec.reshape(
            self.projection_dimensionality, -1, order="F"
    ...
    def _check_input(self, inp):
        ...
        if self.check_input_onehot:
            ...
            if not np.all(mask):
                raise ValueError(
                    " check_input_onehot=True was given in __init__ but the input data has values that are not 0 or 1"
                )
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            ...
            full_dK_dW = (
                kernel_value[:, :, None, None] * extra_term
            )  # n_x, n_x, projection_dim, input_dim
    ...
    def vector_matmul(cls, W, X):
        ...
        x_blowup = X[
            :, :, :, None
        ]  # represent the vectors as a input_dim x 1 matrix (i.e. column vector
    ...
    def outer_vector_product(cls, a):
        ...
        blown_up_a_left = a[
            :, :, :, None
        ]  # now a's as column vector in the last 2 dimensions
        blown_up_aT_right = a[
            :, :, None, :
        ]  # a^T as row vector in the last 2 dimensions
